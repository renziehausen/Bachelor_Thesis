...
gamma = 0.8
phaseFlipList = [qt.basis(2,0)*qt.basis(2,0).dag()+math.sqrt(1-gamma)*qt.basis(2,1)*qt.basis(2,1).dag(),math.sqrt(gamma) * qt.basis(2,0)*qt.basis(2,1).dag()]#alten Namen hier einfach beibehalten
...
def randomNetwork(qnnArch, numTrainingPairs):
    ...
    networkTrainingData = TrainingData(phaseFlipList, numTrainingPairs)
...
def testaverage(numberOfRunsToAverage,maxPairs,untrainedTestData):
    testStates = TrainingData(phaseFlipList,untrainedTestData)#Liste mit nicht trainierten Datenpunkten
    outputList = [[],[],[]]
...
network121lea = randomNetwork([1,2,1], 10)
plotlist121a = qnnTraining(network121lea[0], network121lea[1], network121lea[2], 1, 0.1, 500)[0]
plotlist121b = qnnTraining(network121lea[0], network121lea[1], network121lea[2], 0.5, 0.1, 500)[0]
plotlist121c = qnnTraining(network121lea[0], network121lea[1], network121lea[2], 10, 0.1, 500)[0]
...
columns = "N,a,b,c"
np.savetxt("Amplitude 1-2-1 network lda05-1-2 eps01.csv",
...
columns = "N,a,b,c,d"
np.savetxt("Amplitudedifferentnetworks121o11o131o1221.csv",
...
network121gen = randomNetwork([1,2,1],10)
...
np.savetxt("Amplitude 1-2-1 10training pairs 20avg 10trainingdata.csv",
...
#np.savetxt("bitflip 1-2-1 training pairs.csv",
#           finallist,
#           delimiter =", ",
##           fmt ='% s')
...
np.savetxt("amplitude damping 1-2-1 network lda05 eps01.csv",
           plotlist121,
           delimiter =", ",
           fmt ='% s')
