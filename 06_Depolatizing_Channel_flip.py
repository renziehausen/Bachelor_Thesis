...
def TrainingData(N):
    ...
    for i in range(N):
        t = randomQubitState(numQubits)
        utu = tensoredId(1)*0.7/2 + (1-0.7)*t
...
def randomNetwork(qnnArch, numTrainingPairs):
    ...
    networkTrainingData = TrainingData(numTrainingPairs)
...
def testaverage(numberOfRunsToAverage,maxPairs,untrainedTestData):
    testStates = TrainingData(untrainedTestData)#Liste mit nicht trainierten Datenpunkten
    outputList = [[],[],[]]
...
columns = "N,a,b,c"
np.savetxt("Depolarizing 1-2-1 network lda05-1-2 eps01.csv",
...
columns = "N,a,b,c,d"
np.savetxt("Depolarizingdifferentnetworks121o11o131o1221.csv",
...
np.savetxt("Depolarizing 1-2-1 10training pairs 20avg 10trainingdata.csv",
...
'''network121 = randomNetwork([1,2,1], 10)
plotlist121 = qnnTraining(network121[0], network121[1], network121[2], 2, 0.1, 500)[0]
...
np.savetxt("Depolarizing 1-2-1 network lda2 eps01.csv",
           plotlist121,
           delimiter =", ",
           fmt ='% s')
